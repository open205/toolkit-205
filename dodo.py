...
def task_build_schema():
    """Build the schema"""
    return {"actions": ["doit -d schema-205"]}
...
def collect_schema_files():
    ...
    for file_name in sorted(os.listdir(SCHEMA_SOURCE_PATH)):
        if ".schema.yaml" in file_name:
            ...
            file_list.append(
                os.path.join(JSON_SCHEMA_PATH, f"{file_name_root}.schema.json")
            )
    return file_list
...
def collect_generated_examples(examples_source_dir, examples_output_dir, extension):
    ...
    for example in sorted(os.listdir(examples_source_dir)):
        ...
        if os.path.isdir(example_source_path):
            file_paths += collect_generated_examples(
                example_source_path, example_output_path, extension
            )
        else:
            base_name = os.path.basename(example)
            file_name = os.path.splitext(base_name)[0]
            file_paths.append(
                os.path.join(examples_output_dir, f"{file_name}.{extension}")
            )
    return file_paths
...
cbor_examples = collect_generated_examples(
    EXAMPLES_SOURCE_PATH, CBOR_OUTPUT_PATH, "cbor"
)
...
yaml_examples = collect_generated_examples(
    EXAMPLES_SOURCE_PATH, YAML_OUTPUT_PATH, "yaml"
)
...
xlsx_examples = collect_generated_examples(
    EXAMPLES_SOURCE_PATH, XLSX_OUTPUT_PATH, "xlsx"
)
...
json_examples = collect_generated_examples(
    EXAMPLES_SOURCE_PATH, JSON_OUTPUT_PATH, "json"
)
...
def task_templates():
    ...
    return {
        ...
        "file_dep": [TEMPLATE_CONFIG]
        + collect_schema_files()
        + [os.path.join(TK205_SOURCE_PATH, "xlsx.py")],
...
def task_web():
    ...
    return {
        ...
        "file_dep": cbor_examples
        + yaml_examples
        + xlsx_examples
        + json_examples
        + template_files,  # Add markdown content
...
def task_libtk205():
    ...
    def configure_build(PAT):
        subprocess.run(
            f"cmake -B {LIB_BUILD_PATH} -DPA_TOKEN={PAT} -DBUILD_LIBTK205=ON -Dlibtk205_BUILD_TESTING=ON",
            cwd=".",
            shell=True,
            check=True,
        )
    ...
    return {
        "params": [{"name": "PAT", "long": "PAT", "default": ""}],
        "actions": [
