...
def generate_page(
    env, template_name, file_name, destination, headline, markdown=None, content=None
):
    """
...
def create_files(web_dir):
    ...
    template_content = tk205.load(
        os.path.join(root_dir, "..", "config", "templates.json")
    )
    for RS, templates in template_content.items():
        for template in templates:
            ...
            tk205.template(
                RS, os.path.join(templates_dir, file_name), **template["keywords"]
            )
    ...
    if os.path.isdir(web_dir) and is_git_repo(web_dir):
        ...
        try:
            pygit2.clone_repository(
                "https://github.com/open205/open205.github.io.git",
                web_dir,
...
def generate(web_dir):
    ...
    for schema_file in sorted(schema_dictionary):
        ...
        title, description = get_title_and_description(
            os.path.join(schema_directory, schema_file)
        )
        title_description_tupel = (title, description)
        if (
            RS != "ASHRAE205"
        ):  # if file is the 205 schema itself, use only the schema title
            title = RS + ": " + title
    ...
    generate_page(
        env,
        "schema_template.html",
        "schema.html",
        web_dir,
        "JSON Schema (Normative)",
        markdown=markdown_html,
        content=schema_page_data,
    )
    ...
    for file_type in examples_dictionary:
        for RS in sorted(examples_dictionary[file_type]):
            for file_name in sorted(examples_dictionary[file_type][RS]):
                ...
                rs_id, description = get_title_and_description(
                    os.path.join(examples_directory, "json", RS, base_name + ".json")
                )
    ...
    for i, (RS, content) in enumerate(template_content.items()):
        ...
        for item in content:
            templates_page_data[title_and_description].append(
                {
                    "title": RS,
                    "description": item["description"],
                    "template_file": templates_dictionary[j],
                }
            )
            j += 1
    ...
    generate_page(
        env,
        "templates_template.html",
        "templates.html",
        web_dir,
        "XLSX Templates",
        markdown=markdown_html,
        content=templates_page_data,
    )
    ...
    generate_page(
        env, "about_template.html", "index.html", web_dir, "", markdown=markdown_html
    )
    ...
    generate_page(
        env, "tk205_template.html", "tk205.html", web_dir, "", markdown=markdown_html
    )
